...
module.exports = function(settings) {
    ...
    mail.schedule = function(req, res) {
        ...
        if(!mail) {
            ...
            if(!mail.time || new Date(mail.time) - now <= 0 || mail.time.toLowerCase() === 'now') { /* TODO should the now case be removed? */
                ...
                /* TODO save the message is the db (message set should
                 * very similar to users - although needs the
                 * functionality to be able to pull all messages from
                 * the db on startup (TODO pull & schedule on startup) */
            }
        }
    };
    ...
    function send(mail, auth, callback) {
        ...
        transport.sendMail(mail, function(error, response){
            ...
            transport.close(); /* TODO It'd be good to batch these for
            * a user if they have multiple to send at once, as
            * re-opening is costly */
        });
        return true; /* not an indication of sending success, just
         * passing validation, callback function will alert to immediate
         * error */
    }
