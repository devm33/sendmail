...
module.exports = function(settings) {
            ...
            if(reminder.key){
                 userlib.loadUserWithKey(reminder.key, function(err, user) {
                    if(err) {
                        ...
                        reminder.user = user;
                        scheduleReminder(reminder, function(err, msg) {
                            utillib.send(req, res, err, msg, 500);
                        });
                    }
                 });
    ...
    function scheduleReminder(reminder, callback){
        if(!reminder){
            callback(true, 'Empty reminder object');
            return;
        }
        if(!reminder.user){
            callback(true, 'No user specified');
            return;
        }

        var sec = Date.parse(reminder.time);
        if(isNaN(sec) || sec - Date.now() <= 0){
            remind(reminder, function(error, response){
                if(error) {
                    callback(true, error);
                }
                else {
                    callback(null, 'reminded');
                }
            });
        }
        else{
           //TODO look into how saving works and how it will interact with the work Dev is doing on edit and cancel capability
           //saveReminder(reminder);
           setTimeout(function(){remind(reminder);}, sec-Date.now());
           callback(null, 'scheduled reminder');
        }
    }
    ...
    /* Remind - Marks specified reminder object as unread and moves it to inbox */
    function remind(reminder, callback){
        ...
        var xoauth2gen = xoauth2.createXOAuth2Generator({
            ...
            clientId: secrets.web.client_id,
            clientSecret: secrets.web.client_secret,
            refreshToken: reminder.user.refresh_token
        });
    ...
    function loadForUser(user_id, callback) {
