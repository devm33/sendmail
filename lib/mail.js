...
module.exports = function(settings) {
    ...
    /*** initialize module */
    /* TODO need to schedule all messages in database: loadAll(cb) */
    ...
    function loadForUser(user_id, callback) {
        redis.smembers('mail:user:'+user_id, function(err, mail) {
            console.log('mail for '+user_id+' loaded. '+mail.length);
            callback(err, mail);
        });
    }
    ...
    function loadAll(callback) {
        redis.smembers('mail', function(err, mail) {
            console.log('all '+mail.length+' mail loaded');
            callback(err, mail);
        });
    }
    ...
    function del(mail) {
        ...
        redis.del(key);
        redis.srem('mail', key);
        redis.srem('mail:user:'+mail.user_id, key);
    }
