...
module.exports = function(settings) {
    ...
    function schedule(mail, callback) {
        ...
        if(!valid(mail)) {
            callback(true, 'E-mail missing some fields. Please fill out all fields and try again.');
            return;
        }
    ...
    function transport(mail, user, callback) {
        ...
        /* Body field needs to be renamed for html emails to render properly */
        mail.html = mail.body;
        delete mail.body;
        ...
        xoauth2gen.getToken(function(err, token){
            ...
            imap.once('ready', function(){
                imap.openBox("[Gmail]/All Mail", false, function(err, mailbox){
                    imap.search([['X-GM-MSGID', parseInt(reminder.gmid, 16)]], function(err, results){
                        if(err){
                            errlog(err);
                            callback(true, err);
                        }
