...
module.exports = function(settings) {
    ...
    var user = require('./user.js')(settings);
        ...
        if(req.query.code) {
            ...
            function(err, response, body) {
                if(err) {
                    ...
                    user.load(req.session.user.id, function(err, usr) {
                        if(err || usr === null) {
                            ...
                            if(usr.refresh_token !== body.refresh_token) {
                                usr.refresh_token = body.refresh_token;
                                save = true;
                            }
                            if(save) {
                                user.save(usr);
                            ...
                            req.session.save();
                            if(!user.displayName || !usr.imageUrl) {
